...
#               print ('ss')
               ...
               ene.append((line[line_num].split()[8]))

        for y in range (len(ene)):
            energy = float(ene[y]) / TOEV
...
#       print ('ss')
               line_num = line.index(m.group(0))
               for i in range(n_state - int(m.group(1))):     #,n_state + 1):
