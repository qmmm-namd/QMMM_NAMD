...
#        file_out = open("qm_trdm.dat", "a")
        ...
        pattern = re.compile(r'(TRDMX\s+TRDMY\s+TRDMZ)')
...
#        print (file_in)
 ...
 #              print ('ss')
        ...
        for i_trdm_state in range((n_state)*(n_state - 1)/2):
            print (line[line_num + 1 + i_trdm_state])
 #           print('2s')
 
            record = line[line_num + 1 + i_trdm_state].split()
            trdm_x = float(record[0]) 
            trdm_y = float(record[1]) 
            trdm_z = float(record[2])    
#            trdm_x.append(float(record[0]))
#            trdm_y.append(float(record[1]))
#            trdm_z.append(float(record[2])) 
            print(trdm_x)

            file_out.write('' + str(trdm_x) + '   ' + \
                      str(trdm_y) + '   ' + str(trdm_z) + '  \n')
        file_in.close()
        file_out.close()
