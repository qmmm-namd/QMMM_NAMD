...
class chemshell_log_parser_2_qchem(object):
    u"""
...
######end get mm energy
    ...
    def get_gradient(self):
        ...
        for i_atom in range(self.n_atom):
             ...
             mm_gradient[i_atom][0] = self.chemshell_gradient[i_atom][0] - self.qm_gradient[0][i_atom][0]
             mm_gradient[i_atom][1] = self.chemshell_gradient[i_atom][1] - self.qm_gradient[0][i_atom][1]
             mm_gradient[i_atom][2] = self.chemshell_gradient[i_atom][2] - self.qm_gradient[0][i_atom][2]
    ...
    def get_chemshell_gradient(self):
        """ get ground state gradient from chemshell output """

        chemshell_gradient = []

        for i_atom in range(self.n_atom):
             chemshell_gradient.append([])

             chemshell_gradient[i_atom].append(0.0)
             chemshell_gradient[i_atom].append(0.0)
             chemshell_gradient[i_atom].append(0.0)
        ...
        for i in xrange(self.n_atom):
            ...
            chemshell_gradient[i][0] = grad_x
            chemshell_gradient[i][1] = grad_y
            chemshell_gradient[i][2] = grad_z

        file_in.close()

        self.chemshell_gradient = chemshell_gradient
    ...
    def get_qm_gradient(self):
        ...
        n_row = qm_atom_number / 6
        n_rem = qm_atom_number % 6
        ...
        for i_row in range(n_row):
           ...
           for i_dim in range(3):
               ...
               for i in range(6):
                   ...
                   current_atom = qm_atom_start-1+i_row*6+i
        ...
        if n_rem > 0:
           ...
           for i_dim in range(3):
               ...
               for i in range(n_rem):
                   ...
                   current_atom = qm_atom_start-1+n_row*6+i
            ...
            while line != "":
                ...
                if m is not None:
                  ...
                  for i_atom in range(qm_atom_number):
                      ...
                      nac[i][j][real_atom][0] = -float(record[1])
                      nac[i][j][real_atom][1] = -float(record[2])
                      nac[i][j][real_atom][2] = -float(record[3])
