...
class molpro_parser:
    ...
    def qm_nac_parse(self):
        ...
        with open(self.out) as out:
            while 1:
                for line in out:
                    if 'SA-MC NACME FOR STATES' in line:
                        break 
                else:
                    break 

                state_list = re.findall('\d+',line)
                assert len(state_list) == 4, line
                i_state, j_state = int(state_list[0]), int(state_list[2])
                ...
                for i_atom in range(self.n_atom_qm):
                    nac = list(map(float, out.readline().strip().split()[-3:]))
                    assert len(nac) == 3
                    self.qm_nac[i_state][j_state].append((np.array(nac)*(-1)).tolist())
                    self.qm_nac[j_state][i_state].append(nac)
        ...
        with open(self.out) as out:
            while 1:
                ...
                for i_atom in range(self.n_atom_mm):
                    nac = list(map(float, out.readline().strip().split()[-3:]))
                    assert len(nac) == 3
                    self.mm_nac[i_state][j_state].append((np.array(nac)*(-1)).tolist())
                    self.mm_nac[j_state][i_state].append(nac)
