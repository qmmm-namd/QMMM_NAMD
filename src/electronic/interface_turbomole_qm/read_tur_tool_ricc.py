...
#   Read RICC output from Turbomole
def read_trdm_ricc(n_state, index_state):
...
#        print (file_in)
    ...
    for x in line:
        ...
        if m1:
#       print ('ssm1')
           line_num = line.index(m1.group(1))
#           print (line[line_num].split()[7])
           trdm.append((line[line_num].split()[7]))

        m2 = pattern2.match(x)
        if m2:
#       print ('ssm2')
           line_num = line.index(m2.group(1))
#           print (line[line_num].split()[3])
           trdm.append((line[line_num].split()[3]))
#print (trdm)
    trdm_x = []
    trdm_y = []
    trdm_z = []
    for i in range(0,(len(trdm)/3)):
        trdm_x.append(trdm[3 * i])
        trdm_y.append(trdm[3 * i + 1])
        trdm_z.append(trdm[3 * i + 2])

    for x in range(n_state):
        for y in range(x+1,n_state):
#        f = x + y*(y - 1)/2
            file_out.write('{0}   {1}  {2:>15} {3:>15} {4:>15} \n'.format(x,y,trdm_x[0],trdm_y[0],trdm_z[0]))
            trdm_x.pop(0)
            trdm_y.pop(0)
            trdm_z.pop(0)
